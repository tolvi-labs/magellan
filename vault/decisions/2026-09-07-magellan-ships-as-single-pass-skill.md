...
- **Drift from the approved spec:** Phase 3's `refs` bullet was widened during implementation (commit `8c2e8bd`) to add a third named source — a pre-existing symbol named only in a task's own title or the upstream brief's `how` — after the build's own scenario-tracing (M8) surfaced that the spec's original two-source wording wouldn't reliably surface such a symbol. This is a refinement of the compile mechanism, not a change to Magellan's scope or architecture.
